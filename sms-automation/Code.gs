...
/**
 * [테스트 0] 뿌리오 연결 테스트 — 문자 발송 없이, 구글 서버에서
 * 뿌리오 인증(토큰 발급)이 되는지만 확인합니다. 비용 0원.
 * 성공하면 IP 제한 문제가 없다는 뜻입니다.
 */
function testPpurioConnection() {
  try {
    getPpurioToken_();
    Logger.log('✅ 뿌리오 연결 성공! IP 제한 문제 없음. 다음 단계로 진행하세요.');
  } catch (e) {
    Logger.log('❌ 뿌리오 연결 실패: ' + e.message);
    Logger.log('오류 내용에 IP 관련 문구가 있으면 뿌리오 상담(070-8707-1635)에 유동 IP 사용 가능 여부를 문의하세요.');
  }
}
